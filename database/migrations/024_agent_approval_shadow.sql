-- Phase 4 shadow-only ExecutionIntent and Approval persistence.
-- This migration does not authorize or execute financial actions.
...
CREATE INDEX IF NOT EXISTS agent_execution_intents_actor_created_idx
  ON public.agent_execution_intents (actor_id, created_at DESC);
CREATE INDEX IF NOT EXISTS agent_approvals_intent_recorded_idx
  ON public.agent_approvals (intent_id, recorded_at DESC);
...
BEGIN
  IF COALESCE(p_record->>'schemaVersion', '') <> 'agent.approval_shadow.v1'
     ...
     OR length(COALESCE(intent_input->>'intentDigest', '')) <> 64 THEN
    RAISE EXCEPTION 'invalid approval shadow record' USING ERRCODE = '22023';
  END IF;
  ...
  INSERT INTO public.agent_execution_intents (
    ...
    (intent_input->>'expiresAt')::timestamptz
  )
  ON CONFLICT (actor_id, intent_digest, status) DO UPDATE
    SET intent_digest = EXCLUDED.intent_digest
  ...
  INSERT INTO public.agent_approvals (
    ...
    intent_input->>'intentDigest',
    p_record->>'status',
    p_record->>'legacyConfirmationKind',
    NULLIF(p_record->>'legacyRequestId', ''),
    true,
  ...
  INSERT INTO public.agent_approval_audit (
    approval_id, intent_id, actor_id, event_type, payload
  ) VALUES (
    persisted_approval.approval_id,
    persisted_intent.intent_id,
    persisted_approval.actor_id,
    ...
    jsonb_build_object(
      ...
      'status', persisted_approval.status,
      'legacy_confirmation_kind', persisted_approval.legacy_confirmation_kind
    )
  );

  RETURN jsonb_build_object(
    'record', p_record
      || jsonb_build_object('approvalId', persisted_approval.approval_id)
      || jsonb_build_object('intent', intent_input
        || jsonb_build_object('intentId', persisted_intent.intent_id)),
    'shadowMode', true
  );
END;
$$;
...
COMMENT ON TABLE public.agent_execution_intents IS
  'Shadow-only exact financial intents. Presence does not grant execution authorization.';
COMMENT ON TABLE public.agent_approvals IS
  'Shadow records of legacy confirmations. Runtime enforcement is intentionally disabled in Phase 4 shadow mode.';

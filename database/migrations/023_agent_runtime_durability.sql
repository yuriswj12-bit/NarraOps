...
-- Apply before enabling recoveryOnStart. Service role only.
...
CREATE INDEX IF NOT EXISTS agent_event_outbox_unpublished_idx
  ON public.agent_event_outbox (outbox_sequence)
...
CREATE INDEX IF NOT EXISTS agent_tool_calls_task_created_idx
  ON public.agent_tool_calls (task_id, created_at);
CREATE INDEX IF NOT EXISTS agent_artifacts_conversation_updated_idx
  ON public.agent_artifacts (conversation_id, updated_at DESC);
...
CREATE OR REPLACE FUNCTION public.agent_append_task_event_v2(
  p_task_id uuid,
  p_event jsonb
) RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  INSERT INTO public.agent_event_outbox (
    ...
    COALESCE(p_event->>'traceId', task_row.request_id),
    p_event,
...
BEGIN
  ...
  IF NOT FOUND
     OR current_task.status <> ALL(p_expected_statuses)
     OR current_task.state_version <> p_expected_version THEN
    RETURN NULL;
  END IF;
  ...
  UPDATE public.agent_tasks SET
    ...
    lease_expires_at = CASE WHEN p_patch ? 'lease_expires_at'
      THEN (p_patch->>'lease_expires_at')::timestamptz ELSE lease_expires_at END,
  ...
  IF p_event IS NOT NULL THEN
    persisted_event := public.agent_append_task_event_v2(p_task_id, p_event);
  END IF;

  RETURN jsonb_build_object(
    'task', to_jsonb(next_task),

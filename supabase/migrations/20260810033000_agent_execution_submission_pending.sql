-- Runtime v2 separates a claimed broadcast attempt from provider acceptance.
-- `submission_pending` persists the exact signed transaction identity before
-- an external call. `submitted` is recorded only after the provider accepts it.

ALTER TABLE public.agent_executions
  DROP CONSTRAINT IF EXISTS agent_executions_status_check;
ALTER TABLE public.agent_executions
  ADD CONSTRAINT agent_executions_status_check CHECK (status IN (
...
BEGIN
  ...
  IF NOT ((current_execution.status, next_status) IN (VALUES
    ('reserved', 'submission_pending'),
    ('reserved', 'failed'),
    ('reserved', 'cancelled'),
    ('submission_pending', 'submitted'),
    ('submission_pending', 'reconciliation_required'),
...
REVOKE ALL ON FUNCTION public.agent_transition_execution_v1(jsonb)
...
COMMENT ON FUNCTION public.agent_transition_execution_v1(jsonb) IS
  'Atomically records fixed execution transitions and audit events; submission_pending is pre-provider and submitted requires provider acceptance.';

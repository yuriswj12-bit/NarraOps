...
export class GmgnExecutionAdapter {
  ...
  async tokenSecurity({ chain = "solana", address, requestId } = {}) {
    ...
    if (!this.enabled) return { status: "disabled", source: "gmgn", operation: "token.security", chain: normalized.name, address, request_id: requestId };
  ...
  async multiSwap({ chain = "solana", accounts = [], inputToken, outputToken, inputAmountByWallet, percentBpsByWallet, slippage = 30, autoSlippage = false, requestId } = {}) {
    ...
    if (!this.enabled) {
      return { status: "disabled", source: "gmgn", operation: "multi_swap", chain: normalized.name, request_id: requestId };
    }
    const args = [
  ...
  async orderGet({ chain = "solana", orderId, requestId } = {}) {
    ...
    if (!this.enabled) return { status: "disabled", source: "gmgn", operation: "order.get", chain: normalized.name, request_id: requestId };

...
import { parseGoInput, shouldOpenLaunchCard } from "../../agents/go-command-parser.ts";
...
test("Chinese natural-language intents route without encoding loss", () => {
  ...
  assert.equal(parseGoInput("给我模板").type, "launch.meme");
  assert.equal(parseGoInput("给我预案").type, "launch.meme");
  assert.equal(parseGoInput("给我发射模板").type, "launch.meme");
  assert.equal(parseGoInput("第二套给我").type, "launch.meme");
  assert.equal(parseGoInput("再来一套").type, "launch.meme");
  assert.equal(shouldOpenLaunchCard("给我模板"), true);
  assert.equal(shouldOpenLaunchCard("第二套给我"), true);
  assert.equal(shouldOpenLaunchCard("介绍自己"), false);
  assert.equal(shouldOpenLaunchCard("我发射过多少个 meme"), false);

...
export function buildPulseExecutionPlan(opportunity, options = {}) {
  ...
  const plan = {
    ...
    launch_checklist: [
      ...
      "Keep signing and broadcasting disabled until explicit approval",
    ],
    ...
    next_actions: [
      ...
      "Keep execution disabled until the launch draft is explicitly confirmed.",
    ],
    executable: false,
  ...
  return {
    ...
    body: {
      ...
      message: {
        role: "assistant",
        content:
          "Built a review-only execution plan from the selected Pulse opportunity. Real launch, transfer, and trade actions stay disabled.",
        suggestion:

...
- 已将已停止运营的 Noxa 全面替换为 Pons。NarraOps 已按公开成功交易核验的工厂地址和 `launchToken` 参数结构实现浏览器钱包直发、Gas 预估与二次确认，不再跳转 Pons 官网；后端仍不接触私钥。
...
- Pons 官方源代码验证、完整 ABI、版本升级公告渠道、安全审计材料，以及图片/IPFS 固定服务。当前工厂合约尚未在 Blockscout 开源验证，生产启用前必须固定代码哈希并建立变更监控。
- Pons 跟买路由的实时报价/签名数据来源。公开成功交易确认买入经过未验证路由并包含动态签名参数，因此禁止复制历史 calldata；执行层已保留 `buildPonsBuyBatch` 接口等待接入官方或经审计的报价服务。
- 钱包基础设施的批量签名服务：服务地址、认证凭证、钱包引用映射、单次策略确认令牌、金额上限、地址白名单和紧急暂停配置。执行层只调用一次 `signAndBroadcastBatch`，不会要求用户逐钱包弹窗确认。
- AI Cooking 图片服务：图像模型供应商与密钥、内容安全策略、图片对象存储、IPFS 固定服务、网关域名、文件大小限制和失败清理策略。前端与 `/api/v1/launch/images` 契约已就位，凭证未提供前接口明确返回未配置状态。
